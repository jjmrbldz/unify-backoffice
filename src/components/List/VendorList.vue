...
export default {
    ...
    methods: {
        handleBetLimitListShow(item) {
            const {gameCode, status, casinoName} = item
            if(status === 1) {
                this.$dialog.open(this.$modalComponent.BetLimit, {
                    props: {
                        header: `${this.$store.getters['languageStore/translate'](`${casinoName}`)} ${this.$store.getters['languageStore/translate'](`Bet Limit`)}`,
                        style: {
                            width: '70vw'
                        },
                        modal: true,
